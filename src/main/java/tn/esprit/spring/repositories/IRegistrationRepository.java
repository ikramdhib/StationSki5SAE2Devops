...
public interface IRegistrationRepository extends JpaRepository<RegistrationDTO, Long> {

    long countByCourseAndNumWeek(CourseDTO course, int numWeek);
    @Query("select reg.numWeek from RegistrationDTO reg " +
            "join InstructorDTO ins " +
    ...
    @Query("select count(distinct r) from RegistrationDTO r " +
